...
export const buildExecutionPlan = (step: any): string[] => {
  ...
  if (source) {
    if (phases.source && expected) plan.push(`在 ${source} 检测到 ${expected}`)
    if (hand) plan.push(`手部进入 ${source}${phases.source && expected ? ` 并接近 ${expected}` : ''}`)
    if (!hand && expected) plan.push(`以 ${source} 中 ${expected} 数量减少 1 作为拿取证据`)
  } else {
    if (phases.source && expected) plan.push(`在可视范围检测到 ${expected}`)
    if (hand) plan.push('检测到目标区域外的新手部动作')
  }

  if (phases.transit && expected) {
    plan.push(`移动过程中持续检测 ${expected}${hand ? ' 与手部接近' : ''}`)
  } else if (hand) {
    plan.push('移动过程中跟踪手部关键点')
  }

  if (phases.target && expected) {
    plan.push(`${target} 中 ${expected} 数量相对本轮基线增加 1`)
    if (hand) plan.push('手部离开物料或目标区域后完成')
  } else if (hand) {
    plan.push(`手部或被跟踪物进入 ${target} 后完成`)
  } else if (expected) {
    plan.push(`被跟踪的 ${expected} 进入 ${target} 后完成`)
  }
...
export const validateVisionStep = (step: any): SopValidationResult => {
  ...
  const invalid = (code: string, message: string): SopValidationResult => ({
    valid: false,
    code,
    message: `步骤 ${id}：${message}`,
    plan,
  })

  if (!target) return invalid('target_region_required', '必须配置目标区域')
  if (!expected && anyObjectPhase) return invalid('object_phase_without_expected_object', '未配置期望目标时，不能启用任何物料检测阶段')
  if (!expected && !hand) return invalid('no_observation_method', '必须配置期望目标或有效手部关键点')

  if (source) {
    if (expected && !hand && (!phases.source || !phases.transit)) {
      return invalid('fixed_source_object_only_requires_source_and_transit', '配置了开始区域但未配置手部时，必须同时启用开始区域和移动过程物料检测')
    }
    if (!expected && anyObjectPhase) {
      return invalid('hand_only_object_phase_conflict', '手部-only 配置必须关闭全部物料检测阶段')
    }
    return { valid: true, code: '', message: '', plan }
  }

  if (expected && !hand && !phases.target) {
    return invalid('free_source_object_only_requires_target', '未配置开始区域且未配置手部时，必须启用目标区域物料检测')
  }
  if (!expected && anyObjectPhase) {
    return invalid('hand_only_object_phase_conflict', '手部-only 配置必须关闭全部物料检测阶段')
  }
